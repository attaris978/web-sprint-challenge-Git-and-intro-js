...
function get20s(arr) {
  return arr.filter(val => val.years.slice(0,2) == 19 && val.years.slice(7,9) == 19).map(val => val.name);
}
...
function addArtist(arr) {
  arr.push(   { 
    ...
    bio: 'I\'m lost. I\'ve gone to find myself. If I should return before I get back, please tell me to wait'
  }  );
  return arr;
}
//addArtist(artists);
...
function lotsOfArt(arr) {
  return arr.filter(val => val.paintings > 100).map(val => val.name);
}
...
*/
function artistInfo(arr,arName){
  return [arr[arr.map(val => val.name).indexOf(arName)].bio];
}
...
function artistByCountry(arr,nationality){
  return arr.filter(val => val.nationality === nationality).map(val => val.name);
}
